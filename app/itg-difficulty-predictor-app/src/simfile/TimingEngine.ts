...
import { findLessOrEqual, rowsToTime } from "./utils";
...
export class TimingEngine {
  ...
  /**
   * Gets the time at which a note placed at the given row would
   * need to be hit.
   */
  timeAt(row: number): number {
  ...
  /**
   * Returns true if a note placed at `row` would be hittable.
   * */
  hittable(row: number): boolean {
