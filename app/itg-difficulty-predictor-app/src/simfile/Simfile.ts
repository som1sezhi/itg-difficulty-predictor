...
import {
  ...
  parseTimeSignatures,
  parseWarps,
  type PartiallyProcessedSegment,
  strToFloat,
} from "./field-parsing";
// import { BeatValues } from "./field-parsing";
import { type MSDRecord, parseMSD } from "./msd";
import {
  BPMSegment,
  StopSegment,
  DelaySegment,
...
export class Simfile {
  ...
  constructor(data: string, format: "sm" | "ssc") {
    ...
    // sort charts by stepstype/difficulty slot
    const stepsTypeKey = (chart: Chart) => {
      switch (chart.stepsType) {
        case "dance-single":
          return 1;
        case "dance-double":
          return 2;
