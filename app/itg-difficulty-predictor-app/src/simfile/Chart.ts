import type { MSDRecord } from "./msd";
import { type NoteType, NoteTypes, type Note, type NoteWithTail } from "./Note";
import {
  parseBPMs,
  parseDelays,
  parseFakes,
      ...
      for (const [lineIdx, line] of measureLines.entries()) {
        ...
        for (let c = 0; c < Math.min(numCols, line.length); c++) {
          ...
          if (acceptedNoteTypes.includes(chr)) {
            const row = beatToRow(4 * (measureIdx + lineIdx / numLines));
            yield {
              row,
              column: c,
